 ...
 * - **filled** — lime fill, shirt number (or a portrait, on the export card),
 *   name caption below, ellipsised at `Size.xiCaption`.
 * - **selected** — lime ring plus a `Size.xiHalo` halo.
 *
...
export interface SlotTokenProps {
  ...
  caption?: string;
  /** A portrait inside the ring, clipped to the circle. Card use; the board draws numbers. */
...
export function SlotToken({
  ...
  const showPortrait = filled && !!portrait;
    ...
    <View style={[styles.wrap, ghosted && styles.ghosted]} pointerEvents="none">
      ...
      <View
        style={[
          ...
          { borderWidth: border },
          selected && !showPortrait && styles.selectedRing,
        ]}>
        {showPortrait ? (
          <Image
            source={{ uri: portrait }}
...
const styles = StyleSheet.create({
  ...
  label: { textAlign: 'center', fontVariant: ['tabular-nums'] },
  caption: {
