...
| Goal / score pushes | **BUILT 2026-08-28, shipped dark** ([0053](./decisions/0053-live-match-push-alerts.md)). `match_goal` / `match_red_card` / `match_full_time`, dispatched from inside the live session's 30s loop — NOT the `cronogol-push` cron, whose 30-minute cadence is hopeless for a goal. Gated by a new `alertGoals` switch (defaults OFF) and by `CRONOGOL_LIVE_PUSH_ENABLED`, which is `false` pending a matchday of dry-run logs. ⚠ LaLiga only — the switch's note says so. ⚠ `time-sensitive` needs the entitlement now in `app.json` or iOS silently downgrades it. Live Activities stay deferred ([0054](./decisions/0054-live-activities-still-deferred.md)). |
