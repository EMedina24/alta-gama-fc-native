...
- **Superseded in part by:** [0086](./0086-week-widget-hero-and-rail.md) — the LEDGER and the POLL, on the **medium** widget alone. YOUR WEEK no longer draws a live row, and `FixtureProvider` now gates both `LiveFetch.fetch()` and the 5-minute cadence on `context.family != .systemMedium` so the medium tile stops spending reloads on a score it does not print. ⚠ Everything below still runs, unchanged, for the small NEXT widget and the Lock Screen accessories — `live.json`, its three writers and the ledger views are untouched.
