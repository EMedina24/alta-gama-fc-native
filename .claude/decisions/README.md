...
| [0048](./0048-live-scores-on-the-today-board.md) | Live scores upgrade Today's in-progress card: `/cronogol/live` gives it a real minute where it can, the ~3h fixture sweep stays the fallback everywhere else, and the scoreboard's suppression is untouched | Accepted — not yet seen against a real in-play match |
